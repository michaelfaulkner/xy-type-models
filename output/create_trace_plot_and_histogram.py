...
def main(config_file, observable_string, length_of_trace_plot=1000, number_of_histogram_bins=1000):
    ...
    # todo iterate over temperatures
    ...
    plt.tight_layout()
    plt.savefig(f"{output_directory}/{observable_string}_vs_time_{int(no_of_sites ** 0.5)}_"
    ...
    plt.savefig(f"{output_directory}/{observable_string}_histogram_{int(no_of_sites ** 0.5)}_"
                f"{int(no_of_sites ** 0.5)}_{algorithm_name.replace('-', '_')}.pdf", bbox_inches="tight")
