...
def get_normalised_power_trispectrum(observable_string, output_directory, temperature_directory, beta, no_of_sites,
    ...
    try:
        ...
        with open(f"{output_directory}/{observable_string}_normalised_power_trispectrum_"
                  f"temp_eq_{temperature:.2f}_delta_f_prime_value.csv", "r") as data_file:
        ...
        for index in range(no_of_auxiliary_frequency_octaves + 1):
            with open(f"{output_directory}/{observable_string}_normalised_power_trispectrum_"
                      f"temp_eq_{temperature:.2f}_f_prime_eq_{index + 1}_x_delta_f_prime.csv", "r") as data_file:
        ...
        for index in range(no_of_auxiliary_frequency_octaves + 1):
            with open(f"{output_directory}/{observable_string}_normalised_power_trispectrum_"
                      f"temp_eq_{temperature:.2f}_f_prime_eq_{index + 1}_x_delta_f_prime.csv", "w") as data_file:
...
def get_normalised_power_trispectrum_zero_mode(observable_string, output_directory, temperature_directory, beta,
    ...
    temperature = 1 / beta
    try:
        with open(f"{output_directory}/{observable_string}_normalised_power_trispectrum_temp_eq_{temperature:.2f}_zero_"
                  f"auxiliary_frequency_mode.csv", "r") as data_file:
            return np.loadtxt(data_file, dtype=float, delimiter=",")
    except IOError:
        if no_of_jobs == 1:
        ...
        with open(f"{output_directory}/{observable_string}_normalised_power_trispectrum_temp_eq_{temperature:.2f}_zero_"
                  f"auxiliary_frequency_mode.csv", "w") as data_file:
