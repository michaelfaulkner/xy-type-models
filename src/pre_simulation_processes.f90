...
write(6, '(A)') 'xy-type-models (version 1.0.0) - a Fortran 90/Python application for event-chain/Metropolis Monte &
                Carlo simulation of the XY model, harmonic XY (HXY) model and Maggs lattice-field electrolyte model &
                (each model defined on a square, two-dimensional lattice; event-chain Monte Carlo (ECMC) only &
                available for the XY and HXY models).'
