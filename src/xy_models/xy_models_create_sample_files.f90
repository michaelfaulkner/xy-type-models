...
write (filename, '(A, F4.2, "//sum_of_1st_deriv_of_potential_sample.dat")' ) &
        trim(output_directory)//trim(temperature_string), temperature
open(unit=11, file=filename)
write (filename, '(A, F4.2, "//sum_of_2nd_deriv_of_potential_sample.dat")' ) &
        trim(output_directory)//trim(temperature_string), temperature
...
write (filename, '(A, F4.2, "//external_minimising_twist_field_sample.dat")' ) &
        trim(output_directory)//trim(temperature_string), temperature
