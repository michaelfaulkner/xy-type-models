...
implicit none
integer :: n, i
double precision :: candidate_spin_value, potential_difference, candidate_emergent_field(4)

call randomise_array_of_sites
do n = 1, no_of_sites
    i = array_of_sites(n)
    candidate_spin_value = mod(spin_field(i) + width_of_proposal_interval * (rand() - 0.5d0), twopi)
    candidate_emergent_field(1) = modulo(candidate_spin_value - spin_field(get_south_neighbour(i)) + pi, twopi) - pi
    candidate_emergent_field(2) = modulo(- candidate_spin_value + spin_field(get_west_neighbour(i)) + pi, twopi) - pi
    candidate_emergent_field(3) = modulo(spin_field(get_north_neighbour(i)) - candidate_spin_value + pi, twopi) - pi
    candidate_emergent_field(4) = modulo(- spin_field(get_east_neighbour(i)) + candidate_spin_value + pi, twopi) - pi
